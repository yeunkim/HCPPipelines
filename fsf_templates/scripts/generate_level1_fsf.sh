...
usage() {
    ...
    echo ""
    echo " Usage ${scriptName} --studyfolder=<study-folder> --subject=<subject-id> --taskname=<task-name> --dir=<direction>\\"
    ...
    echo "                    (e.g. EMOTION_LR, tEMOTION_RL, tWM_LR, WM_RL, etc.)"
    ...
    echo "   <direction>    - phase encoding direction"
    echo ""
    echo ""
...
main() {
    ...
    taskfolder=`ls -d ${StudyFolder}/${Subject}/MNINonLinear/Results/*${taskname}*${PEdir}*`
    tasknamebids=`basename ${taskfolder}`
    taskfile=${StudyFolder}/${Subject}/MNINonLinear/Results/${tasknamebids}/${tasknamebids}.nii.gz
    ...
    fsf_template_file=`ls ${templatedir}/*${taskname}*${PEdir}*`

    # copy the template file to the intended destination FSF file
    cp -p ${fsf_template_file} ${outdir}/${tasknamebids}_hp200_s4_level1.fsf
